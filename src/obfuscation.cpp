    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Trip and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 TRIP per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeTripAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
        nFeeRet, strFail, &coinControl, ONLY_NONDENOMINATED_NOTCOLLATERALIFMN);
    if (!success) {
        // if we failed (most likeky not enough funds), try to use all coins instead -
        // MN-like funds should not be touched in any case and we can't mix denominated without collaterals anyway
        CCoinControl* coinControlNull = NULL;
        LogPrintf("MakeCollateralAmounts: ONLY_NONDENOMINATED_NOTCOLLATERALIFMN Error - %s\n", strFail);
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, strFail, coinControlNull, ONLY_NOTCOLLATERALIFMN);
        if (!success) {
            LogPrintf("MakeCollateralAmounts: ONLY_NOTCOLLATERALIFMN Error - %s\n", strFail);
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100TRIP+1 ( bit on if present )
    // bit 1 - 10TRIP+1
    // bit 2 - 1TRIP+1
    // bit 3 - .1TRIP+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
            if (out.nValue == GetSporkValue(SPORK_20_COLLATERAL) * COIN) {
