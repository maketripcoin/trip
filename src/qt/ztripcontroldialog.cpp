...
#include "ui_ztripcontroldialog.h"
...
std::list<std::string> ZTripControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTripControlDialog::listMints;

ZTripControlDialog::ZTripControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTripControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTripControlDialog::~ZTripControlDialog()
{
    delete ui;
}

void ZTripControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTripControlDialog::updateList()
{
...
void ZTripControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTripControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTrip_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTripControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTripControlDialog::GetSelectedMints()
{
...
void ZTripControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
