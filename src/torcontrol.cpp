 ...
 * the server reply formats for PROTOCOLINFO (S3.21) and AUTRIPHALLENGE (S3.24).
 */
 ...
 * Grammar is implicitly defined in https://spec.torproject.org/control-spec by
 * the server reply formats for PROTOCOLINFO (S3.21), AUTRIPHALLENGE (S3.24),
...
private:
    ...
    /** Callback for AUTRIPHALLENGE result */
    void autriphallenge_cb(TorControlConnection& conn, const TorControlReply& reply);
 ...
 *
 *    After a controller sends a successful AUTRIPHALLENGE command, the
...
void TorController::autriphallenge_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        LogPrint("tor", "tor: SAFECOOKIE authentication challenge successful\n");
        std::pair<std::string,std::string> l = SplitTorReplyLine(reply.lines[0]);
        if (l.first == "AUTRIPHALLENGE") {
            std::map<std::string,std::string> m = ParseTorReplyMapping(l.second);
            if (m.empty()) {
                LogPrintf("tor: Error parsing AUTRIPHALLENGE parameters: %s\n", SanitizeString(l.second));
                return;
            }
            std::vector<uint8_t> serverHash = ParseHex(m["SERVERHASH"]);
            std::vector<uint8_t> serverNonce = ParseHex(m["SERVERNONCE"]);
            LogPrint("tor", "tor: AUTRIPHALLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));
            ...
            LogPrintf("tor: Invalid reply to AUTRIPHALLENGE\n");
        }
    } else {
    ...
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                ...
                _conn.Command("AUTRIPHALLENGE SAFECOOKIE " + HexStr(clientNonce), boost::bind(&TorController::autriphallenge_cb, this, _1, _2));
            } else {
