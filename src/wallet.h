...
enum AvailableCoinsType {
    ...
    ONLY_NONDENOMINATED_NOTCOLLATERALIFMN = 4, // ONLY_NONDENOMINATED and not GetSporkValue(SPORK_20_COLLATERAL) CLAB at the same time
    ONLY_COLLATERAL = 5,                        // find clabnode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTRIP send
enum ZerocoinSpendStatus {
    ZTRIP_SPEND_OKAY = 0,                            // No error
    ZTRIP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTRIP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTRIP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTRIP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTRIP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTRIP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTRIP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTRIP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTRIP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTRIP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTRIP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTRIP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTRIP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTRIP_SPENT_USED_ZTRIP = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZTripAutoBackups(bool fEnabled)
    {
